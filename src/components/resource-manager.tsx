...
import { createResourceAction, deleteResourceAction, removeMediaAction, updateResourceAction } from "@/app/[locale]/(dashboard)/dashboard/actions";
...
async function ResourceFields({ resource, row, categories, events, goals }: { resource: ResourceName; row?: ResourceRecord; categories: Option[]; events: Option[]; goals: Option[] }) {
  ...
  return <>
    ...
    <label className="form-wide"><span>{t("fields.images")}</span><input className="form-input file-input" type="file" name="images" accept="image/jpeg,image/png,image/webp,image/avif" multiple={resource !== "categories"} /><small>{t("fields.imageHint")}</small></label>
  </>;
}
